...
class Task(models.Model):
    ...
    def _calculate_planned_dates(self, date_start, date_stop, user_id=None, calendar=None):
        ...
        if not calendar:
            user = self.env['hr.employee'].sudo().browse(user_id) if user_id and user_id != self.env.user.id else self.env.user
    ...
    def schedule_tasks(self, vals):
        ...
        if (vals.get('user_ids') and len(vals['user_ids']) == 1) or ('user_ids' not in vals and len(self.user_ids) == 1):
            user = self.env['hr.employee'].browse(vals.get('user_ids', self.user_ids.ids))
    ...
    def gantt_unavailability(self, start_date, end_date, scale, group_bys=None, rows=None):
        ...
        tag_user_rows(rows)
        resources = self.env['hr.employee'].browse(user_ids).mapped('resource_ids').filtered(lambda r: r.company_id.id == self.env.company.id)
    ...
    def _gantt_progress_bar_user_ids(self, res_ids, start, stop):
        ...
        users = self.env['hr.employee'].search([('id', 'in', res_ids)])
